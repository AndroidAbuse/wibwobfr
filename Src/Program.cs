...
class Program
{
    static void Main(string[] args)
    {
        ...
        // Only use HTTPS redirection if explicitly enabled (disabled on Render behind reverse proxy)
        if (string.Equals(builder.Configuration["UseHttpsRedirection"], "true", StringComparison.OrdinalIgnoreCase))
        {
        ...
        AssignDefault(app);
        // Bind to PORT env var (injected by Render) or default to 8080
    ...
    static void AssignGameServerHandlers(WebApplication app)
    {
        ...
        app.MapPost("/evolveYoukai.nhn", async ctx =>
        {
            await EvolveYoukaiHandler.HandleAsync(ctx);
        });
